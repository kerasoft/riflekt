    ...
    <div className={`inset-0 bg-[#222a] z-10 ${showCart ? 'fixed' : 'hidden'}`}>
        <div ref={wrapperRef} className='absolute w-full sm:w-[540px] right-0 bg-[#fefefe] top-0 bottom-0 shadow-md px-1 sm:px-4 overflow-scroll'>
            <div className="flex justify-between mt-5">
              <div onClick={()=>{setShowCart(false)}} className='text-[1.2rem] inline-flex items-center gap-2 cursor-pointer'>
              ...
              }} className='text-[1.6rem] mr-2 cursor-pointer'><BsCartX fill='#f02a34'/></span>
            </div>
          
          <div className='flex flex-col min-h-[84vh]'>
            {cartItems.length < 1 && (<div className='flex flex-col place-items-center mt-12 lg:mt-24'>
              <FiShoppingBag className='text-[6rem] lg:text-[7.5rem]' />
              <p className='text-[1.4rem] lg:text-2xl mt-4'>Cart is empty</p>
              <Link href='/'><a onClick={()=>{setShowCart(false)}} className='mt-8 text-[#fefefe] px-10 py-3 text-[1.1rem] lg:text-[1.2rem] bg-[#f02a34] font-[500] rounded-md'>Continue Shopping</a></Link>
            </div>)}
                ...
                <div className='mt-10 sm:pl-5' key={item._id}>
                  <div className='flex gap-4 sm:gap-12'>
                    ...
                    <div className='self-center'>
                      ...
                      <div className='text-[1.4rem] inline-flex w-[8.65rem] justify-between items-center mt-4'>
                        <span onClick={()=>{
                        ...
                        }} className='cursor-pointer p-[.95rem] py-1 border-[1px] text-[red] border-gray-400'>-</span>
                        <span className='text-[1.3rem]'>{item.quantity}</span>
                        <span onClick={()=>{
                          let updatedCart = cartItems.map((itm, idx)=>{
                            if(idx == i) {
                              setTotalQty(totalQty + 1)
                          ...
                          setCartItems(updatedCart)
                        }} className='cursor-pointer p-[.95rem] py-1 border-[1px] text-[green] border-gray-400'>+</span>
                      </div>
                    </div>
                      <div onClick={()=>{
